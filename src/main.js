...
const randomAlbum = async () => {
  ...
  document
    .querySelector("#get-next-album-btn")
    .addEventListener("click", randomAlbum);
};

// Register on click event handler for login button if exists
document
  .querySelector("#login-btn")
  ?.addEventListener("click", redirectSpotifyAuthEndpoint, false);

// Register on click event handler for get album button if exists
document
  .querySelector("#get-album-btn")
  ?.addEventListener("click", randomAlbum);
